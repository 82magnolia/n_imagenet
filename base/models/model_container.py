...
class ModelContainer(ABC):
    ...
    def infer(self, target_model: str, input_tensor: torch.Tensor, no_grad: bool, **kwargs):
        """
    ...
    def complex_infer(self, target_model: str, input_dict: dict, no_grad: bool, **kwargs):
        """
    ...
    def print_model_size(self, target: list):
        ...
        """
        for tgt_model in target:
            print('Number of model parameters in {}: {}'.format(tgt_model, sum([p.data.nelement() for p in self.models[tgt_model].parameters()])))
            print('Model size of {}: {} MB'.format(tgt_model, round(4 * sum([p.data.nelement() for p in self.models[tgt_model].parameters()]) / 1024**2)))
